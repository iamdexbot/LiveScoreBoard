/* =========================================
   BASKETBALL SCOREBOARD — script.js
   ========================================= */
...
// Lighten a hex color by `amount` for panel2
...
function openThemeModal() {
  buildThemeSwatches();
  // highlight current preset if applicable
  document.querySelectorAll('.theme-swatch').forEach(function(sw) {
...
function uploadLogo(event, team) {
  ...
  reader.onload = function(e) {
    ...
    if (team === 'home') {
      ...
      showLogo('home', dataUrl);
    } else {
      awayLogoData = dataUrl;
      showLogo('away', dataUrl);
    }
    syncState();
  };
...
function showLogo(team, dataUrl) {
  const img         = document.getElementById(team + 'LogoImg');
  const placeholder = document.getElementById(team + 'LogoPlaceholder');
  const removeBtn   = document.getElementById(team + 'LogoRemove');
  img.src = dataUrl;
...
// ── LocalStorage sync (for viewer.html) ──
function syncState() {
...
function toggleTimer() {
  ...
  if (timerRunning) {
    ...
    // also start shot clock if not already running
...
document.getElementById('minsModal').addEventListener('click', function(e) {
  if (e.target === this) closeMinsEditor();
});
...
function buildDots() {
  ['home', 'away'].forEach(function(team) {
    ...
    for (let i = 0; i < MAX_FOULS; i++) {
      ...
      dot.addEventListener('click', (function(t, idx) {
        return function() { toggleFoul(t, idx); };
      })(team, i));
...
function buzzer() {
  ...
  try {
    ...
    // NBA-style: two-layer foghorn — a deep sawtooth + mid square, 1.1s blast
    function makeHorn(freq, type, startTime, duration, vol) {
      const osc  = ctx.createOscillator();
      const gain = ctx.createGain();
      // slight vibrato via a second oscillator modulating frequency
      ...
      // pitch sags slightly at end like a real horn
      osc.frequency.linearRampToValueAtTime(freq * 0.94, startTime + duration);

      gain.gain.setValueAtTime(0, startTime);
      gain.gain.linearRampToValueAtTime(vol, startTime + 0.03);   // quick attack
    ...
    // Short gap then a second blast (like NBA end-of-quarter)
...
function closeLeagueEditor() {
  document.getElementById('leagueModal').classList.remove('open');
}
...
document.getElementById('leagueModal').addEventListener('click', function(e) {
  if (e.target === this) closeLeagueEditor();
});
...
updateShotClockDisplay();
document.getElementById('timer').textContent = formatTime(timerCentiseconds);
applyTheme(THEMES[0], false);   // apply default theme without overwriting state
syncState();
